...
EarthShaderClass::EarthShaderClass()
{
	m_Effect = 0;
	m_Technique = 0;
	m_Layout = 0;
...
EarthShaderClass::EarthShaderClass(const EarthShaderClass& other)
{
...
bool EarthShaderClass::Initialize(ID3D10Device *device, HWND hwnd)
{
	if (!InitializeShader(device, hwnd, "Data/Shaders/EarthShader.fx"))
		return false;

	return true;
}

bool EarthShaderClass::InitializeShader(ID3D10Device *device, HWND hwnd, char *shaderFilename)
{
	HRESULT result;
	...
	m_Technique = m_Effect->GetTechniqueByName("EarthTechnique");
	if (!m_Technique)
		return false;
...
void EarthShaderClass::OutputErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, char *shaderFilename)
{
	char* compileErrors;
...
void EarthShaderClass::Render(ID3D10Device* device, unsigned long indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D10ShaderResourceView** texturesArray,
	D3DXVECTOR3 lightDirection, D3DXCOLOR diffuseColor)
{
	SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, texturesArray, lightDirection, diffuseColor);
...
void EarthShaderClass::SetShaderParameters(D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D10ShaderResourceView** texturesArray,
	D3DXVECTOR3 lightDirection, D3DXCOLOR diffuseColor)
{
	m_worldMatrixPtr->SetMatrix((float*)&worldMatrix);
	m_viewMatrixPtr->SetMatrix((float*)&viewMatrix);
	m_projectionMatrixPtr->SetMatrix((float*)&projectionMatrix);
	m_texturesArrayPtr->SetResourceArray(texturesArray, 0, 5);
...
void EarthShaderClass::RenderShader(ID3D10Device* device, unsigned long indexCount)
{
...
void EarthShaderClass::Shutdown()
{
	ShutdownShader();
...
void EarthShaderClass::ShutdownShader()
{
	if (m_Effect)
	{
		m_Effect->Release(); 
