#ifndef EARTHSHADERCLASS_H
#define EARTHSHADERCLASS_H
...
class EarthShaderClass
{
public:
	EarthShaderClass();
	EarthShaderClass(const EarthShaderClass&);
	~EarthShaderClass();

	bool Initialize(ID3D10Device *device, HWND hwnd);
	void Render(ID3D10Device* device, unsigned long indexCount,
		D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix,
		ID3D10ShaderResourceView** texturesArray, D3DXVECTOR3 lightDirection, D3DXCOLOR diffuseColor);
	void Shutdown();
...
private:
	...
	void SetShaderParameters(D3DXMATRIX, D3DXMATRIX, D3DXMATRIX, ID3D10ShaderResourceView**, D3DXVECTOR3, D3DXCOLOR);
